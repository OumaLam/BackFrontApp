...
const VaccinationList = () => {
 ...
 const router = useRouter();
   ...
   const newVaccin = `/${basePath}/`;
  useEffect(() => {
    setIsClient(true);

    const fetchVaccinations = async () => {
      try {
        const response = await axios.get("http://localhost:8080/api/vaccinations",{ withCredentials: true });
  ...
  const handleDelete = async (idVaccin) => {
    if (confirm("Êtes-vous sûr de vouloir supprimer ce vaccin ?")) {
      try {
        await axios.delete(`http://localhost:8080/api/vaccinations/${idVaccin}`,{ withCredentials: true });
  ...
  return (
    <div className="max-w-4xl mx-auto p-8  ">
      ...
      <button
          onClick={() => router.push(newVaccin+'Vaccination/new')}
      ...
      {vaccinations.length === 0 ? (
        ...
        <div className="space-y-6 mt-6">
          {currentVaccinations.map((vaccination) => (
            <div key={vaccination.idVaccin} className="p-6 bg-white rounded-lg shadow-md">
              <div className="flex justify-between items-center mb-2">
                ...
                <button
                    onClick={() =>
                      router.push(newVaccin+`Vaccination/animaux-non-vaccines/${vaccination.idVaccin}`,)
                    }
                  ...
                  <button
                    onClick={() =>
                      router.push(newVaccin+`Vaccination/edit/${vaccination.idVaccin}`)
                    }
                    className="text-blue-600 hover:text-blue-800"
                    title="Modifier"
                  >
                 <FaEye />
                  ...
                  <button
                    onClick={() =>
                      router.push(newVaccin+`Vaccination/details/${vaccination.idVaccin}`)
                    }
