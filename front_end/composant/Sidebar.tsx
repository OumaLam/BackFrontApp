...
export default function DashboardLayout({ children, menuItems }) {
  ...
  return (
    <div className="fixed w-64 flex h-screen bg-gray-100">
      {/* Sidebar */}
      <aside className="fixed md:relative z-40 top-0 left-0 w-64 h-screen bg-[#2d775c] text-white flex flex-col justify-between p-4">
        <div>
          {/* Logo */}
          <div className="flex items-center space-x-3 bg-green-50 rounded-xl p-3 mb-6">
            <img src="/favicon.ico" alt="Logo" className="w-10 h-10" />
            <span className="font-bold text-[#2d775c] text-sm">Ranch Adarouch</span>
          </div>
          ...
          <nav className="space-y-2">
            {menuItems.map((item, index) => (
              <a
                key={index}
                href={item.href}
                className={`w-62 flex items-center px-4 py-3 rounded-l transition-all duration-200 ${
                  pathname === item.href
      ...
      {/* Modal */}
      {showLogoutModal && (
        <div className="fixed inset-0 z-50 bg-[#2d775c] bg-opacity-0 flex items-center justify-center">
