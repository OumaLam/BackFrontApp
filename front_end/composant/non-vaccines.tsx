...
const VaccinationConfirmation = () => {
  ...
  useEffect(() => {
    const fetchAnimals = async () => {
      try {
        const res = await axios.get(`http://localhost:8080/api/vaccin/animaux/non-vaccines?vaccinId=${vaccinId}`,{ withCredentials: true });
        setAnimals(res.data);
 ...
 const handleSearch = (e) => {
  ...
  const confirmVaccination = async () => {
    setShowModal(false);
    try {
      await axios.post(`http://localhost:8080/api/vaccin/animal-vaccinations/${vaccinId}`, { 
